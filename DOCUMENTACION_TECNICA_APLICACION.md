...
"Buenos días. Quiero presentar una aplicación que hemos desarrollado para automatizar completamente el proceso de seguimiento del Microsoft 365 Roadmap. Esto que antes requería revisiones manuales, ahora se ejecuta automáticamente cada mes."

### 2. PROBLEMA (1 minuto)

"El desafío era: Microsoft publica constantemente nuevas funcionalidades, cambios y mejoras en su roadmap. Antes, alguien tenía que:

1. Entrar al sitio de Microsoft
2. Revisar manualmente qué era nuevo
3. Seleccionar lo relevante para nosotros
4. Redactar un email
5. Enviarlo al equipo

Todo esto manualmente, cada mes, con posibilidad de errores."

### 3. SOLUCIÓN (1 minuto)

"Ahora:

1. La aplicación se conecta automáticamente a Microsoft Release Communications
2. Descarga TODOS los cambios recientes
3. Filtra automáticamente por productos que nos interesan
4. Evita duplicados recordando qué ya procesamos
5. Genera un informe visual profesional
6. Lo envía por email

Todo esto sin intervención humana, de forma consistente y auditable."

### 4. ARQUITECTURA GENERAL (2 minutos)

"Técnicamente funciona así:

[Mostrar diagrama]

La aplicación se comunica con Microsoft mediante MCP - Model Context Protocol. Esto NO es IA, es simplemente una forma estandarizada de acceder a datos estructurados.

Recibe aproximadamente 370 elementos cada mes, los filtra hasta quedar con 20-30 relevantes, genera un informe HTML profesional y lo envía."

### 5. TECNOLOGÍAS (1 minuto)

"Usamos:

- **Python** para lógica principal - es simple, mantenible, multiplataforma
- **SQLite** para almacenamiento local - no necesita servidor, es seguro
- **MCP (Streamable HTTP)** para conectar con Microsoft - protocolo estandarizado
- **SMTP de Office 365** para envío - usa contraseña de aplicación, seguro

Todo simple, sin dependencias externas complejas."

### 6. FLUJO DE DATOS (2 minutos)

"[Mostrar diagrama de flujo completo]

En detalle:

1. **Conexión:** Conectamos al servidor MCP de Microsoft
2. **Descarga paginada:** Bajamos elementos en lotes de 50
3. **Filtro de fecha:** Guardamos solo de los últimos 30 días
4. **Filtro de producto:** Solo Exchange, Teams, SharePoint, OneDrive
5. **Deduplicación:** Consultamos SQLite - ¿Ya lo procesamos?
6. **Obtención de detalles:** Si es nuevo, bajamos toda la info
7. **Procesamiento:** Enriquecemos datos, traducimos, formateamos
8. **HTML:** Generamos informe visual
9. **Envío:** SMTP a destinatarios

Todo registrado en logs para auditoría."

### 7. SEGURIDAD (1 minuto)

"Seguridad:

- Credenciales en variables de entorno, nunca en código
- Contraseña de aplicación de Office 365, no la normal
- STARTTLS para encriptación en tránsito
- Logs no contienen secretos
- DRY_RUN para testear sin enviar emails reales
- Backups automáticos de base de datos

OAuth 2.0 está preparado para futuro cuando tengamos permisos Azure AD."

### 8. PRUEBAS (1 minuto)

"Para probar seguro:

1. Configuramos DRY_RUN=true
2. Ejecutamos la aplicación
3. Se genera el HTML de preview
4. NO se envía email
5. SQLite NO se modifica
6. Podemos revisar exactamente qué se enviaría"

### 9. AUTOMATIZACIÓN (1 minuto)

"Para ejecutar automáticamente cada mes:

- **Windows:** Task Scheduler (cron de Windows)
- **Linux:** Cron job estándar
- Se ejecuta a la hora que definamos
- Logs se guardan automáticamente
- Si falla, podemos ver exactamente qué fue"

### 10. DEMOSTRACIÓN (Interactiva)

Si tienes acceso a la app:
...
### 11. PREGUNTAS ESPERADAS (1 minuto cada una)

**P: ¿Qué pasa si falla Internet?**

A: Se registra el error, se intenta de nuevo automáticamente con wait progresivo, y si sigue fallando se detiene con mensaje claro en logs.

**P: ¿Se puede enviar dos veces el mismo email?**

A: No. SQLite recuerda cada elemento procesado, y aunque se ejecute dos veces, solo envía nuevos.

**P: ¿Cómo hacemos cambios sin romper todo?**

A: DRY_RUN permite testear antes de ejecutar en producción. Además tenemos tests automáticos.

**P: ¿Quién mantiene esto?**

A: Cualquier técnico puede mantenerlo. El código está documentado, logs son claros, y la arquitectura es simple.

---

## PREGUNTAS FRECUENTES

### ¿Por qué Python y no JavaScript/C#?

**Python:**
- ✅ Ecosistema excelente para scripting
- ✅ Bibliotecas maduras para HTTP/async
- ✅ Fácil de mantener y leer
- ✅ Cross-platform
- ✅ Facilita análisis de datos futuro

### ¿Por qué MCP y no RSS?
...
### ¿Por qué no Gemini/IA?

**Razones:**
- ✅ Reglas determinísticas = auditable
...
**IA sería útil si:**
- Necesitáramos clasificación inteligente
- Hubiera criterios subjetivos
- Quisiéramos resumen con lenguaje natural

### ¿Qué pasa si Microsoft cierra MCP?

**Poco probable pero:**
- El descubrimiento dinámico hace fácil migrar
- RSS es fallback
- Graph API tiene otros endpoints
- Con 2-3 días de trabajo se adaptaría

### ¿Se puede ampliar a más productos?

**Sí, fácil:**

```python
# Modificar src/config.py línea 94-99
ALLOWED_PRODUCTS = {
    "Exchange Online",
    "Microsoft Teams",
    "SharePoint",
    "OneDrive",
    "Power BI",           # Nuevo
    "Defender for 365",   # Nuevo
}
```

Luego ejecutar con DRY_RUN=true para probar.

### ¿Qué parte está automatizada?
...
❌ MANUAL (por ahora):
- Activación/ejecución (se configura automático después)
- Cambios de configuración (editando .env)

### ¿Cómo manejar si necesita reautenticación?

**Si usar OAuth 2.0:**
- MSAL maneja esto automáticamente
- Si token expira, se refreshea silenciosamente
- Si necesita login interactivo, avisa al usuario

**Con SMTP actual:**
- Contraseña almacenada en .env
- Si falla auth, lo registra en logs

### ¿Cuál es el costo operativo?

- 🔴 Microsoft MCP: Gratis (es servicio público)
- 🔴 SMTP Office 365: Gratis (incluido en licencia)
- 🔴 SQLite: Gratis (local)
- 🔴 Hosting: 0 € si se ejecuta en PC/server existente
- ✅ **Costo total: $0**

### ¿Cómo se documenta el proceso?

1. **Código:** Comentarios en secciones complejas
2. **Logs:** Detalles de cada paso de ejecución
3. **Documentación:** Este archivo, README.md, archivos de configuración
4. **Tests:** Validan comportamiento esperado

### ¿Es fácil entregarle a otros equipos?

**Para que otros equipos lo usen:**

1. Clonan el repositorio
2. Crean .env con sus credenciales
3. Ejecutan `python main.py`
4. Listo

O si queremos automatización:

5. Configuran Task Scheduler / Cron
6. Se ejecuta automáticamente cada mes
...
**API (Application Programming Interface)** - Conjunto de reglas para que un programa se comunique con otro. Ej: MCP es una API.

**Async/Await** - Forma de programar que permite que varias tareas ocurran "simultáneamente" sin bloquear. Python lo usa con asyncio.

**Backup** - Copia de seguridad de datos. La aplicación crea backups de SQLite antes de modificar.

**Caché** - Almacenamiento rápido de datos recientemente usado. MCP cachea lista de herramientas.

**CORS (Cross-Origin Resource Sharing)** - No aplica aquí (es para web). Mencionado por completitud.

**CRUD** - Create, Read, Update, Delete. Operaciones básicas en base de datos.

**CSV** - Formato de datos en tabla (valores separados por comas). No usado en esta app.

**Despliegue (Deployment)** - Proceso de poner una aplicación en producción.

**DRY (Don't Repeat Yourself)** - Principio: No repetir código. NO es nuestro "DRY_RUN".

**DRY_RUN** - Modo de prueba: ejecuta todo pero sin efectos permanentes (sin enviar email).

**EHLO** - Extended HELLO en SMTP. Comando que inicia comunicación.

**Endpoint** - URL específica de una API. Ej: `https://www.microsoft.com/releasecommunications/mcp`

**Encriptación** - Convertir datos en código ilegible. STARTTLS lo hace.

**Enriquecimiento de datos** - Agregar información a datos existentes. DataProcessor lo hace.

**Entorno Virtual (venv)** - Directorio aislado con Python y librerías específicas. Evita conflictos.

**Exit Code** - Número que un programa retorna al terminar. 0=éxito, 1=error.

**Filtrado** - Seleccionar datos que cumplen criterios. Por fecha, por producto.

**Git** - Sistema de control de versiones. Trackea cambios en código.

**Gitignore** - Archivo que dice a Git qué NO guardar. `.env` está aquí.

**GUID (Globally Unique Identifier)** - ID único que identifica un recurso. Lo usa Microsoft.

**Hash** - Función que convierte datos en número único. Para verificar integridad.

**HTTP** - Protocolo para transferir datos web.

**HTTPS** - HTTP con encriptación (el "S" = Secure).

**JSON** - Formato para datos estructurados. `{"key": "value"}`.

**JSON-RPC** - Protocolo donde se envía JSON diciendo qué función ejecutar remotamente.

**Lag** - Retraso. Si MCP es lento, hay lag.

**Logging** - Registro de eventos. `logger.info()`, `logger.error()`.

**ORM (Object-Relational Mapping)** - Librería que mapea objetos Python a SQL. SQLAlchemy es una. No usamos (usamos SQL directo).

**OAuth 2.0** - Estándar moderno de autenticación. Más seguro que contraseña + usuario.

**OAuthenticate** - Proceso de obtener token OAuth. No implementado aún.

**Open Source** - Código disponible públicamente. Nuestro código NO es open source (es privado).

**Parser** - Programa que analiza formato de datos. MCP parser lee JSON.

**Payload** - Datos enviados en una petición HTTP.

**Port (Puerto)** - Número que identifica servicio en computadora. 587 para SMTP, 443 para HTTPS.

**Query (SQL)** - Comando para consultar/modificar base de datos.

**Rate Limiting** - Limitar velocidad de peticiones. Nuestra app espera 0.1s entre peticiones MCP.

**Regex (Regular Expression)** - Patrón para buscar/reemplazar texto. No usado mucho aquí.

**RFC (Request for Comments)** - Documento que define un estándar. RFC para SMTP, JSON-RPC, etc.

**Runtime** - Tiempo mientras se ejecuta un programa.

**Schema (Base de datos)** - Estructura de tablas, columnas, tipos de datos.

**Servidor** - Computadora que corre un servicio que otros acceden.

**Session** - Conexión mantenida entre cliente y servidor. HTTPx no mantiene session (stateless).

**SHA-256** - Algoritmo de hash. MSAL lo usa para seguridad de tokens.

**SMTP (Simple Mail Transfer Protocol)** - Protocolo estándar para enviar emails.

**Socket** - Conexión de red entre dos programas.

**SQL** - Lenguaje para bases de datos. SQLite es una implementación de SQL.

**SQLite** - Base de datos embebida, un archivo local, sin servidor. Usamos: `data/processed_items.db`.

**SSL/TLS** - Encriptación de conexión. STARTTLS inicia TLS.

**Stakeholder** - Persona interesada en el proyecto. Managers, usuarios finales.

**STARTTLS** - Comando SMTP que inicia encriptación sobre conexión existente.

**Status Code HTTP** - Número que indica resultado. 200=OK, 404=No encontrado, 500=Error servidor.

**String** - Texto/cadena de caracteres.

**Subroutine** - Pequeña función/procedimiento (término antiguo).

**Timestamp** - Marcas de tiempo. La app las usa en SQLite.

**Token** - Credencial temporal que prueba identidad. OAuth los usa.

**Transacción** - Operación de BD: All-or-nothing. Si falla, se revierta todo.

**Trigger** - En BD: Regla que se ejecuta automáticamente. Ej: actualizar timestamp.

**UTC (Coordinated Universal Time)** - Hora estándar internacional. Las fechas de Microsoft usan UTC+00:00.

**UUID** - Synonym de GUID. ID único universal.

**Variable de Entorno** - Dato configurado en SO, accesible a programas. .env expone como variable.

**XML** - Formato de datos similar a JSON (más antiguo). RSS usaba XML.

**Webhook** - URL que se invoca cuando algo ocurre. No lo usamos (sería para futuro).
...
"Una aplicación Python que automatiza completamente el seguimiento y distribución del Microsoft 365 Roadmap mediante descarga filtrada, deduplicación en SQLite y envío de email profesional."

### En 30 Segundos

"Automatizamos el análisis del roadmap de Microsoft. Cada mes, nuestra aplicación descarga las novedades de Exchange, Teams, SharePoint y OneDrive, filtra por relevancia, genera un informe HTML profesional y lo envía automáticamente al equipo. Sin intervención manual, consistencia garantizada, auditable en logs."

### En 2 Minutos

"**Problema:** Antes revisábamos manualmente el Microsoft 365 Roadmap cada mes.

**Solución:** Desarrollamos una aplicación que:

1. Se conecta automáticamente a Microsoft Release Communications (MCP)
2. Descarga ~370 elementos de roadmap
3. Filtra los últimos 30 días
4. Selecciona solo 4 productos relevantes (Exchange, Teams, SharePoint, OneDrive)
5. Consulta SQLite para evitar duplicados
6. Obtiene detalles completos de nuevos elementos
7. Genera un informe visual en HTML
8. Lo envía por email

Todo en ~25 segundos, sin errores, sin duplicados, completamente auditable.

**Tecnologías:** Python, MCP, SQLite, SMTP Office 365.

**Próximos pasos:** OAuth 2.0, automatización mensual, dashboard web."

### En 10 Minutos

[Usar el "Guion para Presentación" anterior]
...
Usar `LOG_LEVEL=DEBUG` en .env para más detalle.
...
1. **Revisar logs:** `Get-Content logs/app.log -Tail 50`
2. **Ejecutar con DEBUG:** `set LOG_LEVEL=debug` luego `python main.py`
3. **Probar con DRY_RUN:** `set DRY_RUN=true` para no enviar email real
4. **Reset DB:** `python reset_database.py` si sospechas corrupción
